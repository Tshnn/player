...
bool VedioFFmpeg::ToRGB(const AVFrame *aframe, uint8_t *m_pixels[], int *m_ptich, int outwidth, int outheight)
{
    ...
    // 线程同步是保护Vavformtctx这些会被多线程访问的成员的
    if (aframe->format < 0)
    {
        vedio_frame_mutex.unlock();
    ...
    cout<<"原像素格式:"<<av_get_pix_fmt_name((enum AVPixelFormat)aframe->format)<<endl;
    ...
    vedio_frame_mutex.unlock();
    // cout<<"现像素格式:"<<av_get_pix_fmt_name((enum AVPixelFormat)frame->->format)<<endl;
